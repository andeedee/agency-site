...
[{sel:'.hero-btns',txt:'No pitch \xb7 No pressure \xb7 30 mins'},{sel:'.cta-finale-btns',txt:'⭐ 4.9 from 247 calls \xb7 \xa350 time guarantee \xb7 30 mins'},{sel:'.about-hero-btns',txt:'No pitch \xb7 No pressure \xb7 30 mins'},{sel:'.about-cta-btns',txt:'⭐ 4.9 from 247 calls \xb7 \xa350 time guarantee'}].forEach(({sel,txt})=>{document.querySelectorAll(sel).forEach(c=>{if(c.nextElementSibling?.classList.contains('cta-micro'))return;const m=document.createElement('p');m.className='cta-micro';m.textContent=txt;c.parentNode.insertBefore(m,c.nextSibling);});});
document.querySelectorAll('.calc-cta-btn').forEach(b=>{if(b.nextElementSibling?.classList.contains('cta-micro'))return;const m=document.createElement('p');m.className='cta-micro';m.style.color='rgba(255,255,255,.75)';m.textContent='No pitch \xb7 30 mins \xb7 \xa350 time guarantee';b.parentNode.insertBefore(m,b.nextSibling);});
document.querySelectorAll('.cs-cta-btn').forEach(b=>{if(b.nextElementSibling?.classList.contains('cta-micro'))return;const m=document.createElement('p');m.className='cta-micro';m.style.color='rgba(61,74,86,.65)';m.textContent='No pitch \xb7 30 mins \xb7 \xa350 time guarantee if not a fit';b.parentNode.insertBefore(m,b.nextSibling);});
...
if(document.body.dataset.page!=='calendar'){const s=document.createElement('div');s.className='sticky-cta';s.innerHTML=`<a href="calendar.html" class="sticky-cta-btn"><span>Get my free 30-min strategy call</span><span class="arrow">→</span></a><div class="sticky-cta-sub">⭐ 4.9 \xb7 \xa350 time guarantee \xb7 No pitch</div>`;document.body.appendChild(s);const r=()=>{if(window.scrollY>600){s.classList.add('visible');window.removeEventListener('scroll',r);}};window.addEventListener('scroll',r,{passive:true});}
